...
export const PanelControl = () => {
  ...
  const fetchUsuarios = async () => {
    try {
      const response = await fetch('http://localhost:3000/users/usuarios');
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await fetch('http://localhost:3000/users/usuarios', {
        method: 'POST',
        headers: {
  ...
  const handleUpdate = async (e) => {
    e.preventDefault();
    try {
      await fetch(`http://localhost:3000/users/usuarios/${editData.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  const handleDelete = async (id) => {
    try {
      await fetch(`http://localhost:3000/users/usuarios/${id}`, {
        method: 'DELETE'
      });
      fetchUsuarios();
    } catch (error) {
